...
// Copyright © 2022 adryd. MIT licensed; see THIRD_PARTY_LICENSES/oneko-LICENSE.txt.
...
(function oneko() {
  ...
  // Keep the decoration off touch-only devices and respect motion preferences.
  ...
  function onAnimationFrame(timestamp) {
    // Stop execution if the neko element is removed from the DOM.
  ...
  function setSprite(name, frameNumber) {
    const sprite = spriteSets[name][frameNumber % spriteSets[name].length];
  ...
  function idle() {
    ...
    // Roughly every 20 seconds, choose a small idle animation.
    if (
      idleTime > 10 &&
  ...
  function frame() {
    ...
    if (idleTime > 1) {
      setSprite("alert", 0);
      // Pause briefly after being alerted before moving.
